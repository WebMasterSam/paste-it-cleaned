...
export default class TOTPSetupComp extends Component<ITOTPSetupCompProps, ITOTPSetupCompState> {
    ...
    showSecretCode(code: any, theme: any) {
        if (!code) return null
        return (
            <div>
                ...
                <Input autoFocus theme={theme} key="totpCode" name="totpCode" onChange={this.handleInputChange} />
            </div>
        )
    }
    ...
    render() {
        ...
        return (
            <FormSection theme={theme}>
                {this.state.setupMessage && <Toast>{I18n.get(this.state.setupMessage)}</Toast>}
