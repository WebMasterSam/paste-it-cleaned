...
export default class Authenticator extends React.Component<IAuthenticatorProps, IAuthenticatorState> {
    ...
    handleAuthEvent(state: any, event: any, showToast = true) {
        if (event.type === 'error') {
    ...
    render() {
        ...
        if (hideDefault) {
            hide = hide.concat([SignIn, ConfirmSignIn, RequireNewPassword, SignUp, ConfirmSignUp, VerifyContact, ForgotPassword, TOTPSetup, Loading])
        }

        let props_children: any[] = []
        ...
        const default_children = [<SignIn federated={federated} />, <ConfirmSignIn />, <RequireNewPassword />, <SignUp signUpConfig={signUpConfig} />, <ConfirmSignUp />, <VerifyContact />, <ForgotPassword />, <TOTPSetup />, <Loading />]
        ...
        return (
            <React.Fragment>
                <div>
                    ...
                    <Wrapper theme={theme}>{render_children}</Wrapper>
                </div>
            </React.Fragment>
        )
    }
