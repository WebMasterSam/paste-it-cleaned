...
export default class RequireNewPassword extends AuthPiece<IAuthPieceProps, IAuthPieceState> {
    ...
    showComponent(theme: any) {
        ...
        return (
            <FormSection theme={theme} data-test={auth.requireNewPassword.section} className="auth-form">
                ...
                <SectionBody theme={theme} data-test={auth.requireNewPassword.bodySection}>
                    <Input autoFocus placeholder={I18n.get('New Password')} theme={theme} key="password" name="password" type="password" onChange={this.handleInputChange} data-test={auth.requireNewPassword.newPasswordInput} />

                    {requiredAttributes.map((attribute: any) => (
                        <Input placeholder={I18n.get(convertToPlaceholder(attribute))} theme={theme} key={attribute} name={attribute} type="text" onChange={this.handleInputChange} />
                    ))}
                </SectionBody>
                <SectionFooter theme={theme}>
                    ...
                    <SectionFooterSecondaryContent theme={theme}>
                        <Link theme={theme} onClick={() => this.changeState('signIn')} data-test={auth.requireNewPassword.backToSignInLink}>
...
function convertToPlaceholder(str: any) {
    return str
        .split('_')
        .map((part: any) => part.charAt(0).toUpperCase() + part.substr(1).toLowerCase())
        .join(' ')
}
