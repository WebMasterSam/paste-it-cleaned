...
export default class SignIn extends AuthPiece<ISignInProps, ISignInState> {
    ...
    showComponent(theme: any) {
        ...
        const hideSignUp = !override.includes('SignUp') && hide.some((component: any) => component === SignUp)
        const hideForgotPassword = !override.includes('ForgotPassword') && hide.some((component: any) => component === ForgotPassword)

        return (
            <FormSection theme={theme} data-test={auth.signIn.section} className="auth-form">
                <SectionHeader theme={theme} data-test={auth.signIn.headerSection}>
                    {I18n.get('Sign in to your account')}
                </SectionHeader>
                <FederatedButtons federated={federated} theme={theme} authState={authState} onStateChange={onStateChange} onAuthEvent={onAuthEvent} />
                <form onSubmit={this.signIn}>
                    <SectionBody theme={theme}>
                        {this.renderUsernameField(theme)}
                        <FormField theme={theme}>
                            <InputLabel theme={theme}>{I18n.get('Password')} *</InputLabel>
                            <Input placeholder={I18n.get('Enter your password')} theme={theme} key="password" type="password" name="password" onChange={this.handleInputChange} data-test={auth.signIn.passwordInput} />
                            {!hideForgotPassword && (
                                <Hint theme={theme}>
                    ...
                    <SectionFooter theme={theme} data-test={auth.signIn.footerSection}>
                        <SectionFooterPrimaryContent theme={theme}>
                            <Button theme={theme} type="submit" disabled={this.state.loading || !this.state.email || !this.state.password} data-test={auth.signIn.signInButton}>
                                {I18n.get('Sign In')}
                            </Button>
                        ...
                        {!hideSignUp && (
                            <SectionFooterSecondaryContent theme={theme}>
                                {I18n.get('No account? ')}
                                <Link theme={theme} onClick={() => this.changeState('signUp')} data-test={auth.signIn.createAccountLink}>
